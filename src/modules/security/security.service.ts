...
export class SecurityService {
  ...
  async removeAllDevices(userId: string, keepCurrentDeviceId?: string) {
  ...
  async updateDeviceActivity(userId: string, deviceId: string, metadata: {
    ipAddress?: string;
    userAgent?: string;
  }) {
    try {
  ...
  async revokeAllSessions(userId: string, keepCurrentTokenId?: string) {
    const where: Prisma.AuthTokenWhereInput = { userId };
  ...
  async getUserAlerts(userId: string, filters: {
    type?: string;
    severity?: string;
    isRead?: boolean;
    page?: number;
    ...
    const where: Prisma.SecurityAlertWhereInput = { userId };
  ...
  async getSecurityScore(userId: string) {
    ...
    if (deviceCount > 5) score -= 10;
    if (disputeCount === 0) score += 5;
    if (disputeCount > 3) score -= 10;
    if (fraudFlaggedOrders > 0) score -= 15;
    if (recentFailedLogins > 3) score -= 10;
    if (user.failedLoginAttempts > 0) score -= 5;
    if (alertCount > 5) score -= 10;
  ...
  parseUserAgent(userAgent: string): { browser: string; osVersion: string; deviceName: string } {
    let browser = 'Unknown';
    ...
    else if (userAgent.includes('Windows')) deviceName = 'PC';

    return { browser, osVersion, deviceName };
  }

  // --- IP Geolocation (basic lookup) ---
  ...
  async getLocationFromIp(ipAddress: string): Promise<string> {
    ...
    try {
      ...
      const data = await response.json() as { status: string; country?: string; regionName?: string; city?: string };
