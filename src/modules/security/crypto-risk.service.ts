...
/**
 * Withdrawal/deposit destination risk screening. Uses only local checks
 * (format, sanctions, address reuse, velocity, amount thresholds, user risk)
 * with zero external provider calls.
 */
@Injectable()
export class CryptoRiskService {
  private readonly logger = new Logger(CryptoRiskService.name);
   ...
   * Called during deposit processing to flag suspicious deposits early.
   */
